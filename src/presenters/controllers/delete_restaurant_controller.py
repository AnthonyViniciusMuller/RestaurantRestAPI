...
from src.core.use_cases import DeleteRestaurant
...
class DeleteRestaurantController(RouteInterface):

    def __init__(self, use_case: Type[DeleteRestaurant]):
        self.delete_restaurant = use_case
    ...
    def route(self, http_request: Type[HttpRequest]) -> HttpResponse:
        ...
        if http_request.params:
            ...
            if "id" in params.keys() and isinstance(params['id'], int):
                response = self.delete_restaurant.by_id(id = params['id'])
            ...
            response = self.delete_restaurant.all()
